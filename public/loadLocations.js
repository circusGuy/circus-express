...
locations.forEach((location, index) => {
  ...
  const startAbbreviatedDay = startDate.toLocaleDateString("en-US", {
    weekday: "short",
  });

  const endMonth = endDate.toLocaleString("default", { month: "long" });
  const endDay = getOrdinalSuffix(endDate.getDate());
  const endAbbreviatedDay = endDate.toLocaleDateString("en-US", {
    weekday: "short",
  });
  ...
  for (let i = 0; i <= getDifferenceInDays(startDate, endDate); i++) {
    ...
    const abbreviatedDay = currentDate.toLocaleDateString("en-US", {
      weekday: "short",
    });
    ...
    if (abbreviatedDay === "Sat" && location.name === "Mill Hall, PA") {
      ...
      showTimes6Div.innerHTML =
        "Sat: &nbsp; 2:30,&nbsp; 5:00&nbsp; & &nbsp;7:30 PM";
    }
